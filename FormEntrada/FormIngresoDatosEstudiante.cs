...
namespace formEntradaUsuario
{
    public partial class FormIngresoDatosEstudiante : Form
    {
        ...
        {
            comboBox_Carrera.Items.Add("1- Tecnicatura en Programacion");
            comboBox_Carrera.Items.Add("2- Enfermeria");
            comboBox_Carrera.Items.Add("3- Administracion");
            comboBox_Carrera.Items.Add("4- Liquidacion de sueldos");
            comboBox_Carrera.Items.Add("5- Contabilidad");
            comboBox_Carrera.Items.Add("6- Periodismo");
            comboBox_Carrera.Items.Add("7- Analista de redes");
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (nombre is not null && apellido is not null && dni > 0 && direccion is not null && telefono > 0 && correo is not null && contrasena is not null
                && validacion.EsDniUnico(lista, dni))
            {
                if (carrera is not null)
                {
                    Estudiante estudiante = new Estudiante(nombre, apellido, correo, contrasena, dni, telefono, direccion, carrera, cambiaContrasenia,turnoSeleccionado);
                    GestorEstudiantes.AgregarEntidad(estudiante);
                    DialogResult = DialogResult.OK;
                    Close();
                }
                else
                {
                    Estudiante estudiante = new Estudiante(nombre, apellido, correo, contrasena, dni, telefono, direccion, cambiaContrasenia, turnoSeleccionado);
                    GestorEstudiantes.AgregarEntidad(estudiante);
                    DialogResult = DialogResult.OK;
                    Close();
                }
